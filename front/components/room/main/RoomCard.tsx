...
const RoomCard: React.FC<IProps> = ({ room, showMap }) => {
    ...
    return (
        <Container showMap={showMap}>
            <Link href={`/room/${room.no}`}>
                <a>
                    <div className="room-card-photo-wrapper">
                        <CustomImage src={`/${room.roominfo.photos[0]}.jpg`} type="img" subClassName="img"/>
                    </div>
                    <div className="room-card-info-texts">
                        <p className="room-card-room-info">
                            {room.roominfo.buildingType} {translatedRoomType} {room.roominfo.location.district}{" "} {room.roominfo.location.city}
                        </p>
                        <p className="room-card-title">{room.roominfo.title}</p>
                        <div className="room-card-text-divider"/>
                        <p className="room-card-price"><b>₩{room.roominfo.price}</b>/1박</p>
                        {!!remainDays && (
                            <p className="room-card-total-price">
                                총 요금: ₩ {makeMoneyString(`${Number(room.roominfo.price) * remainDays}`)}
                            </p>
                        )}
                    </div>
                </a>
